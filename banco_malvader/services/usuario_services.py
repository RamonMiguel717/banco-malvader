from banco_malvader.repository.usuario_dao import UsuarioRepository as User
from banco_malvader.utils.validator import Validator
from banco_malvader.utils.criptografia_senha import criptografada
from banco_malvader.utils.enviar_email import enviar_otp_email
from banco_malvader.utils.exceptions import ValidacaoNegocioError, AcessoNegadoError
...
class UsuarioServices:
    ...
    def atualizar_dados(id_usuario, telefone=None, email=None, senha=None):
        try:
            ...
            User.update_usuario(id_usuario, telefone=telefone, email=email, senha_hash=senha)
    ...
    def autenticar_usuario(cpf, senha):
        try:
            ...
            usuario = User.get_usuario_by_cpf(cpf)
            if not usuario:
            ...
            if usuario["senha_hash"] != senha_criptografada:
                User.registrar_login(usuario["id_usuario"], sucesso=False)
            ...
            User.registrar_login(usuario["id_usuario"], sucesso=True)
            return usuario
    ...
    def gerar_e_enviar_otp(id_usuario):
        ...
        u.otp_codigo = otp #Atuliza o OTP no banco de dados
    ...
    def validar_otp(id_usuario, otp_digitado):
        try:
            if User.validar_otp(id_usuario, otp_digitado):
    ...
    def tentativas_invalidas_recentemente(id_usuario):
        try:
            total = User.tentativas_recentes_falhas(id_usuario)
            return total
        except Exception as e:
