...
class Validator:
    ...
    def validate_idade(data_nascimento_str: str) -> dict:
        erros = []
        try:
            # Utiliza a função tratar_data para garantir o formato correto
            data_tratada = tratar_data(data_nascimento_str)
            data_nascimento = datetime.strptime(data_tratada, "%Y-%m-%d").date()
            ...
            erros.append(f"Data de nascimento inválida. {str(e)}")
    ...
    @staticmethod
    def validate_senha(senha: str, email: str, nome: str) -> dict:
        erros = []
        if len(senha) < 8:
